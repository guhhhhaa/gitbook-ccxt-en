...
* Private API
  ...
  * `createOrder` → `watchCreateOrder` _\(notice the_ _`watch`_ _prefix\)_
  * `cancelOrder` → `watchCancelOrder` _\(notice the_ _`watch`_ _prefix\)_
...
**Some exchanges do not offer a** _**pub**_ **WS API, they will offer** _**sub**_ **WS API only.** However, there are exchanges that have a complete Streaming API as well. In most cases a user cannot operate effectively having just the Streaming API. Exchanges will stream public market data _sub_, and the REST API is still needed for the _pub_ part where missing.
...
CCXT Pro is designed for async/await style syntax and relies heavily on async primitives such as _promises_ and _futures_.
